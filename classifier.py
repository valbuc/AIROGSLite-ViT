...
from src.transformations import Equalize, EqualizeIgnoreBlack
...
class ClassifierDataset(Dataset):
    def __init__(self, data_dir, start_idx, end_idx, cache_all=False, transform=None):
        ...
        self.data = {}
        labels = pd.read_csv(
            os.path.join(data_dir, "img_info_with_labels.csv"), index_col=0
        )
        ...
        if self.cache_all:
            for i, fn in enumerate(self.filenames):
                ...
                img = Image.open(filepath).convert(
                    "RGB"
                )  # convert forces the image to load in the main process
...
class MyDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument("--normalize_stats", choices=["ViT", "ImgNet"])
        parser.add_argument("--equalize", choices=["no", "yes", "IgnoreBlack"])
    ...
    def __init__(self, args):
        ...
        if args.normalize_stats == "ImgNet":
            self.normalize = transforms.Normalize(
                mean=feature_extractor.image_mean,
                std=feature_extractor.image_std
            )
        elif args.normalize_stats == "ViT":
            self.normalize = transforms.Normalize(
                mean=[0.5, 0.5, 0.5],
                std=[0.5, 0.5, 0.5]
            )
        ...
        if args.equalize == "no":
                    ...
                    transforms.RandomApply(
                        [
                            transforms.RandomAffine(
                                degrees=10, translate=None, scale=(1.0, 1.2)
                            )
                        ],
                        p=0.5,
                    ),
            ...
            self.test_transforms = transforms.Compose(
                [transforms.ToTensor(), self.normalize, transforms.RandomHorizontalFlip(),]
            )
        elif args.equalize == "yes":
            self.train_transforms = transforms.Compose(
                [
                    Equalize(),
                    transforms.ToTensor(),
            ...
            self.test_transforms = transforms.Compose(
                [Equalize(), transforms.ToTensor(), self.normalize, transforms.RandomHorizontalFlip(),]
            )
        elif args.equalize == "IgnoreBlack":
            self.train_transforms = transforms.Compose(
                [
                    EqualizeIgnoreBlack(9, 69)(),
                    transforms.ToTensor(),
                    self.normalize,
                    transforms.RandomHorizontalFlip(),
                    transforms.RandomApply(
            ...
            self.test_transforms = transforms.Compose(
                [EqualizeIgnoreBlack(9, 89)(), transforms.ToTensor(), self.normalize, transforms.RandomHorizontalFlip(),]
            )

        self.labels = pd.read_csv(
            os.path.join(args.data_dir, "img_info_with_labels.csv"), index_col=0
        )
        ...
        self.train_ds = ClassifierDataset(
            args.data_dir,
            cache_all=True,
            start_idx=0,
        ...
        if args.use_validation_set_for_test:
            ...
            self.test_ds = ClassifierDataset(
                args.data_dir,
...
class LitClassifier(pl.LightningModule):
    ...
    def common_step(self, batch, metric_category):
        ...
        for name, metric in metrics.items():
            ...
            self.log(
                f"{metric_category}_{name}", metric_value, on_step=False, on_epoch=True
            )
    ...
    def on_train_start(self):
        ...
        self.logger.log_hyperparams(
            self.hparams, {f"test_{k}": 0 for k in self.metrics["test"].keys()}
        )
...
def cli_main():
    ...
    parser.add_argument(
        "--es_var",
        choices=["val_f1", "val_loss", "val_partial_auroc", "val_auroc"],
        default="val_partial_auroc",
    )
